...
export default function renderComponentList(
  ...
  Object.entries(entrypointsWithMetadata).forEach(([name, entrypoint]) => {
    const filePath = entrypoint.filePath.replace(
      new RegExp(`^${prefixPath}`),
      ''
    );
    ...
    entrypoint.components.forEach(component => {
      ...
      const isDefaultExportRender = component.isDefaultExport
        ? chalk.gray.italic(`(default)`)
        : '';
      console.info(
        indentation(indentation(`${symbolRender} ${isDefaultExportRender}`))
      );
    });
  });
}

export function getComponentCountText(
